...
void run_declaration(const ast_declaration *d, env *e) {
	if (d->kind == AST_GLOBAL) {
		...
		return;
	} else if (d->kind == AST_EXTERN) die("extern is only allowed when compiling");
...
value run_primary(ast_primary *prim, env *e){
	...
	case AST_FNCALL: {
		int kind = 0;
		if (prim->prim->kind == AST_VAR && !strcmp(prim->prim->str, "print")) kind = 1;
		else if (prim->prim->kind == AST_VAR && !strcmp(prim->prim->str, "push")) kind = 2;
		else if (prim->prim->kind == AST_VAR && !strcmp(prim->prim->str, "pop")) kind = 3;
		else if (prim->prim->kind == AST_VAR && !strcmp(prim->prim->str, "length")) kind = 4;
	...
	case AST_ARY:;
		...
		return ary2value(a);
	case AST_VAR:
		if ((v1 = lookup_var(e, prim->str)) == VUNDEF)
			die("undefined variable '%s' accessed", prim->str);

		return v1;
	case AST_INT:
		return num2value(prim->num);

	case AST_STR:
		return str2value(prim->str);

	case AST_TRUE: return VTRUE;
	case AST_FALSE: return VFALSE;
	case AST_NULL: return VNULL;
	}
}
...
int run_block(ast_block *block, value *ret, env *e) {
	...
	for (int i = 0; i < block->amnt; ++i) {
		...
		case AST_LOCAL:
			declare_local(e, s->varname, s->expr ? run_expression(s->expr, e) : VNULL);
			break;
